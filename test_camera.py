            ...
            if result.get('success') and result.get('devices'):
                ...
                if len(devices) > 0:
                    ...
                    if result.get('success'):
                        ...
                        print("\n✅ 测试完成！检查 storage/video/ 目录是否有 test_video.mp4")
                    else:
                        print(f"❌ 录制启动失败: {result.get('error')}")
                else:
                    print("❌ 没有找到摄像头设备")
            else:
                print(f"❌ 枚举失败: {result.get('error')}")
