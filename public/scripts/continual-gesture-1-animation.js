 ...
 * 
 * 这个文件专门负责连续手势1（continual_gesture_1）任务的Stage动画
 * 主要是手指相关的连续动作
 * 
 * 动画特点：
 * - 提示从右向左滚动
...
(function() {
    ...
    class ContinualGesture1AnimationController {
        constructor() {
            ...
            this.gestureTypes = {
                'finger_spread': { label: '手指张合', icon: '✋', color: '#10b981' },
                'finger_tap': { label: '手指点击', icon: '👆', color: '#10b981' },
                'finger_extend': { label: '手指伸展', icon: '🖐', color: '#10b981' },
                'finger_curl': { label: '手指弯曲', icon: '✊', color: '#10b981' }
            };
        }

        loadConfig() {
            if (window.COLLECTION_CONSTANTS && window.COLLECTION_CONSTANTS.CONTINUAL_GESTURE_1) {
                const taskConfig = window.COLLECTION_CONSTANTS.CONTINUAL_GESTURE_1;
        ...
        drawStageInfo() {
            ...
            ctx.fillText(`连续动作: ${gesture.label}`, 20, 20);
        ...
        drawPrompt(prompt) {
            ...
            const radius = 35;
            const gap = 12;
            const cy = y - (this.config.promptLength / 2) - gap - radius;
